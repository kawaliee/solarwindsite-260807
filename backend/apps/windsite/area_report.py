...
def build_area_report(result: dict, evals: list | None = None, *,
    ...
    # ⚠️ 종전 문구는 "풍황은 실측 대상이라 이 검토에 포함되지 않는다"였다.
    #    「발전사업 세부허가기준 등에 관한 고시」 개정으로 풍력 발전사업허가에
    #    풍황계측기 설치가 필수가 아니게 되고 **기상청 재현바람장 자료 제출**로
    #    갈음하게 되면서, 그 문장은 사실과 달라졌다.
    if prof.has_wind_resource:
        limits.append(
            '풍황은 기상청 재현바람장으로 산출했습니다(PART 2 참고). 고시 개정에 '
            '따라 발전사업허가 단계에서는 이 자료로 풍황계측기 설치를 갈음할 수 '
            '있으나, **사업 투자 판단과 발전량 예측에는 여전히 현장 실측이 '
            '필요합니다** — 재현바람장은 모델 재현값이고 격자 대표값입니다.')
    else:
        limits.append(
            '풍황은 태양광 검토 대상이 아닙니다.')
    for t in limits:
