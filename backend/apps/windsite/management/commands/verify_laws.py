...
NOT_A_STATUTE = {
    ...
    # 실측(2026-08) — 법령 검색은 물론 행정규칙 검색('풍력'·'환경성평가 지침'
    # 등 여러 표기)에서도 같은 이름이 나오지 않았다. 환경부가 발간·운영하는
    # 지침이라 국가법령정보에 등재되지 않는다. 근거로 쓰지 말라는 뜻이 아니라
    # 환경부 자료로 직접 대조하라는 뜻이다(레코드의 source_url 참고).
    '환경성평가 지침': '환경부 발간 지침 — 국가법령정보에 등재되지 않는다',
}
