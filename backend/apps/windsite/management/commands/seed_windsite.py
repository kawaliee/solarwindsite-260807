...
LAWS = [
    ...
    dict(name='매장유산 보호 및 조사에 관한 법률', category='문화재',
         purpose='매장유산 유존지역의 보호 (지표조사 의무는 국가유산영향진단법으로 이관)',
         key_articles='제6조의2(국가 등에 의한 매장유산 지표조사) / 제9조(지표조사에 따른 '
                      '매장유산 유존지역의 보호) / 시행령 제4조(매장유산 유존지역의 보호)',
         confidence='HIGH', source_url='https://www.law.go.kr',
         note='구 「매장문화재 보호 및 조사에 관한 법률」에서 법령명 개정. '
              '2024-02-13 개정으로 사업시행자 지표조사 의무(구 제6조)가 삭제되어 '
              '「국가유산영향진단법」 제9조로 옮겨갔다.'),
    dict(name='국가유산영향진단법', category='문화재',
         purpose='건설공사 시행자의 국가유산 영향진단·약식영향진단 의무',
         key_articles='제9조(영향진단의 대상 및 시기) / 제17조(약식영향진단의 대상 등)',
         confidence='HIGH', source_url='https://www.law.go.kr',
         note='2025-02-14 시행. 종전 매장유산법의 사업시행자 지표조사 의무를 이어받았다.'),
    ...
    dict(name='해상풍력 보급 촉진 및 산업 육성에 관한 특별법', category='전기',
         ...
         key_articles='제19조(발전지구의 지정) / 제17조(민관협의회 구성ㆍ운영) / 제25조(실시계획의 승인 등)',
...
STEPS = [
    ...
    dict(order=40, phase='PERMIT', name='재해영향평가 협의',
         ...
         statutory_days=None, statutory_basis='NONE',
         depends_on=['발전사업허가'], conditional_on='', confidence='HIGH',
         note='협의 절차라 법에 처리기간 규정이 없다. 대상 종류·규모는 시행령 제6조제1항 '
              '별표1이 정하므로 사업 규모 확정 후 대조해야 한다. 산사태위험지 포함 시 '
              '중요도 상승.'),
    ...
    dict(order=53, phase='PERMIT', name='사방지 지정해제',
         authority='산림청장 / 시·도지사', law='사방사업법',
         article='제20조(사방지의 지정해제 등)',
         statutory_days=None, statutory_basis='UNKNOWN',
         depends_on=['산지전용허가 또는 산지일시사용허가'],
         conditional_on='FOREST', confidence='HIGH',
         note='사방지가 사업구역에 포함될 때만 해당한다. 지정 근거는 제4조(사방지의 지정).'),
    dict(order=60, phase='PERMIT', name='국가유산 영향진단 (매장유산 지표조사 포함)',
         authority='국가유산청 / 시장·군수·구청장(약식영향진단)',
         law='국가유산영향진단법',
         article='제9조(영향진단의 대상 및 시기) · 제17조(약식영향진단의 대상 등)',
         statutory_days=None, statutory_basis='UNKNOWN',
         depends_on=['발전사업허가'], conditional_on='HERITAGE', confidence='HIGH',
         note='2024-02-13 개정으로 **사업시행자의 지표조사 의무 조항(구 제6조)이 삭제**되고, 그 의무가 「국가유산영향진단법」 제9조(영향진단의 대상 및 시기)로 옮겨갔다. 현행 매장유산법 제6조의2는 국가·지방자치단체가 하는 지표조사이고 제7조도 그 보고서 제출만 남았다 — 사업시행자 근거로 쓸 수 없다. 매장유산 유존지역에서 시행하는 건설공사는 제9조제1항제2호로 영향진단 대상이며, 대상에 해당하지 않는 역사문화환경 보존지역 공사는 같은 법 제17조의 약식영향진단을 시장·군수가 실시한다. 대상 규모는 같은 법 시행령으로 확정해야 한다.'),
    ...
    # ── 국토계획법 경로는 둘 중 하나를 밟는다(법 제56조제1항 단서 —
    #    도시·군계획사업에 의한 행위는 개발행위허가를 받지 않는다).
    #
    #    ⚠️ 용도지역이 가르는 것은 **의무 여부**이지 선택 가능 여부가 아니다.
    #       시행령 제35조제1항제2호 나목은 도시지역 밖에서 전기공급설비를
    #       "결정 없이 설치**할 수 있다**"는 **면제**이지 도시계획시설로
    #       결정하지 말라는 금지가 아니다. 그래서 도시지역 밖에서도 사업자가
    #       도시계획시설 경로를 **선택**할 수 있고, 실무에서 실제로 그렇게
    #       한다(삼척 천봉풍력 — 22기 전부 농림지역인데 도시관리계획
    #       결정고시로 추진).
    #
    #       선택하는 까닭은 실시계획 인가에 붙는 두 가지다.
    #         법 제92조  관련 인·허가등의 의제 — 산지전용(13호)·농지전용(8호)·
    #                    도로점용(9호)·하천점용(26호)·초지전용(23호)·사방지
    #                    지정해제(12호)·무연분묘 개장(10호)이 한꺼번에 의제된다.
    #                    아래 로드맵의 상당수가 이 하나로 갈음된다.
    #         법 제95조  토지 등의 수용·사용 — 사유지가 많은 풍력에서 결정적이다.
    dict(order=69, phase='PERMIT',
         name='도시·군관리계획 결정 (도시계획시설 — 전기공급설비)',
         authority='시·도지사 또는 대도시 시장 (입안: 시장·군수)',
         law='국토의 계획 및 이용에 관한 법률',
         article='제43조제1항 · 시행령 제35조제1항 · 제88조(실시계획의 작성 및 인가)',
         statutory_days=None, statutory_basis='NONE',
         depends_on=['발전사업허가'], conditional_on='URBAN_AREA',
         confidence='HIGH',
         note='풍력발전시설은 국토계획법상 기반시설인 「전기공급설비」다'
              '(시행령 제2조제1항제3호 유통·공급시설). 법 제43조제1항 본문은 기반시설을 '
              '설치하려면 미리 도시·군관리계획으로 결정하라고 하고, 그 예외를 정한 '
              '시행령 제35조제1항제1호 가목의 목록에는 **전기공급설비가 없다.** '
              '따라서 도시지역(녹지지역 포함)·지구단위계획구역에 걸치면 이 경로가 '
              '**의무**다. '
              '⚠️ 도시지역 밖이라도 이 경로를 **선택할 수 있다.** 시행령 제35조제1항'
              '제2호 나목은 결정 없이 설치"할 수 있다"는 면제이지 금지가 아니다. '
              '선택하는 까닭은 실시계획 인가에 붙는 **법 제92조 인허가 의제**'
              '(산지전용·농지전용·도로점용·하천점용·초지전용·사방지 지정해제가 '
              '한꺼번에 의제된다)와 **법 제95조 수용·사용권**이다. 사유지가 많은 '
              '풍력에서는 이 둘이 개발행위허가보다 유리해 실무에서 널리 쓰인다'
              '(삼척 천봉풍력 — 22기 전부 농림지역인데 도시관리계획 결정고시로 추진). '
              '민간이 시행자로 지정받으려면 법 제86조제5항·제7항과 시행령 제96조제2항에 '
              '따라 **대상 토지(국공유지 제외) 면적의 3분의 2 이상 소유 + 토지소유자 '
              '총수 2분의 1 이상 동의**가 필요하다. '
              '결정·인가에 법정 처리기간 규정은 없다(주민의견 청취·관계기관 협의·'
              '도시계획위원회 심의를 거치므로 실무상 가장 긴 절차다).'),
    dict(order=70, phase='PERMIT', name='개발행위허가',
         ...
         statutory_basis='',
         depends_on=['산지전용허가 또는 산지일시사용허가'],
         conditional_on='NON_URBAN_AREA',
         confidence='HIGH',
         note='도시지역 밖(관리·농림·자연환경보전지역)의 전기공급설비는 시행령 '
              '제35조제1항제2호 나목에 따라 도시·군관리계획 결정 없이 설치할 수 있어 '
              '이 경로로 진행한다. 허가·불허가 처분 기간은 15일이나 **도시계획위원회 '
              '심의를 거치거나 관계 행정기관과 협의해야 하는 경우 그 기간은 제외**되므로'
              '(시행령 제54조제1항 괄호) 실제 소요는 훨씬 길다. '
              '⚠️ 면적 상한(시행령 제55조제1항 — 관리·농림 3만㎡, 자연환경보전·보전녹지 '
              '5천㎡ 미만)이 있으나, 풍력발전기는 공작물이라 같은 조 제3항제3의2호 나목'
              '(하나의 용도에 사용되는 공작물 설치를 위한 형질변경)으로 도시계획위원회 '
              '심의를 거쳐 규모 제한을 받지 않을 수 있다 — 면적 초과가 곧 도시계획시설 '
              '경로를 강제하는 것은 아니다. '
    ...
    dict(order=140, phase='OPS', name='REC 발급 신청 및 전력거래 개시',
         ...
         statutory_days=None, statutory_basis='UNKNOWN',
         depends_on=['사업개시신고'], conditional_on='',
         confidence='HIGH'),
]
