...
def _landuse_route_note(doc, ctx) -> None:
    ...
    **애초에 갈림길이 있었다는 사실 자체가 문서에서 사라진다.**
    ...
    rows = []
    for st, when, basis in (
            (non_urban, '도시지역·지구단위계획구역 **밖**\n(관리·농림·자연환경보전지역)',
             '시행령 제35조제1항제2호 나목 —\n「궤도 및 전기공급설비」는 결정 없이 설치'),
            (urban, '도시지역(녹지지역 포함)\n또는 지구단위계획구역에 걸침',
             '법 제43조제1항 본문 —\n예외 목록(제1호 가목)에 전기공급설비가 없음')):
        rows.append([
            st.get('name') or '-',
            '✔ 이 사업에 적용' if st.get('applicable') else '해당 없음',
            when, basis,
        ])
    _styled_table(doc, ['경로', '적용 여부', '어느 때', '근거'], rows,
                  accent='C07A1E', status_col=1, widths=[4.4, 2.4, 4.2, 6.4])
    _note(doc,
          '※ 두 경로는 함께 밟는 것이 아니라 **택일**입니다 — 도시계획시설로 '
          '결정하면 법 제56조제1항 단서에 따라 개발행위허가를 받지 않고, '
          '실시계획 인가(법 제88조)로 갈음합니다. 풍력발전시설이 국토계획법상 '
          '기반시설인 「전기공급설비」(시행령 제2조제1항제3호)이기 때문에 생기는 '
          '분기입니다. **가르는 것은 용도지역이며 설비용량이나 사업면적이 '
          '아닙니다.** 부지 경계를 조정해 도시지역에 걸치면 경로가 통째로 '
          '바뀌므로 부지 확정 전에 재확인하십시오.')
