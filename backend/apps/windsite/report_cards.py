...
def _landuse_route_note(doc, ctx) -> None:
    ...
    **애초에 갈림길이 있었다는 사실 자체가 문서에서 사라진다.**
    ...
    # 각주로만 적었더니 처리기간 안내문 뒤에 묻혀 읽히지 않았다(실측 —
    # 삼척 보고서에 문구는 실렸으나 표만 보아서는 갈림길이 있는 줄 알 수
    # 없었다). 표를 보는 자리에서 갈림길이 보이도록 **작은 대조표**로 낸다.
    doc.add_heading('국토계획법 인허가 경로 (택일)', level=3)
    in_urban = bool(urban.get('applicable'))
    # ⚠️ 용도지역이 가르는 것은 **의무 여부**이지 선택 가능 여부가 아니다.
    #    시행령 제35조제1항제2호 나목은 도시지역 밖에서 "결정 없이 설치할 수
    #    있다"는 **면제**이지 도시계획시설로 결정하지 말라는 금지가 아니다.
    #    도시지역 밖에서도 이 경로를 고를 수 있고 실무에서 실제로 고른다 —
    #    이것을 '해당 없음'으로 적었더니 실제와 어긋났다(삼척 천봉풍력:
    #    22기 전부 농림지역인데 도시관리계획 결정고시로 추진 중).
    rows = [
        [non_urban.get('name') or '-',
         '불가' if in_urban else '가능 (원칙)',
         '도시지역·지구단위계획구역 밖\n(관리·농림·자연환경보전지역)',
         '시행령 제35조제1항제2호 나목 —\n「궤도 및 전기공급설비」는 결정 없이 설치'],
        [urban.get('name') or '-',
         '의무' if in_urban else '선택 가능',
         '도시지역(녹지지역 포함)·지구단위계획구역이면 의무\n'
         '그 밖의 지역에서도 사업자가 선택 가능',
         '법 제43조제1항 본문 —\n예외 목록(제1호 가목)에 전기공급설비가 없음'],
    ]
    _styled_table(doc, ['경로', '이 사업에서', '어느 때', '근거'], rows,
                  accent='C07A1E', status_col=1, widths=[4.2, 2.2, 5.0, 6.0])
    _note(doc,
          '※ 두 경로는 함께 밟는 것이 아니라 **택일**입니다 — 도시계획시설로 '
          '결정하면 법 제56조제1항 단서에 따라 개발행위허가를 받지 않고 '
          '실시계획 인가(법 제88조)로 갈음합니다.'
          + ('' if in_urban else
             ' 이 사업은 도시지역 밖이라 **개발행위허가가 원칙**이지만, '
             '도시계획시설 경로를 택할 수도 있습니다. 실무에서 이 경로를 고르는 '
             '까닭은 실시계획 인가에 붙는 **법 제92조 관련 인·허가등의 의제**'
             '(산지전용·농지전용·도로점용·하천점용·초지전용·사방지 지정해제가 '
             '한꺼번에 의제되어 아래 개별 절차 상당수를 갈음합니다)와 '
             '**법 제95조 토지 등의 수용·사용권**입니다. 사유지가 많은 부지에서는 '
             '이 둘이 결정적입니다. 다만 민간이 시행자로 지정받으려면 법 제86조'
             '제5항·제7항과 시행령 제96조제2항에 따라 **대상 토지(국공유지 제외) '
             '면적의 3분의 2 이상 소유 + 토지소유자 총수 2분의 1 이상 동의**가 '
             '필요합니다.')
          + ' 용도지역이 가르는 것은 **의무 여부**이며, 설비용량이나 사업면적은 '
            '이 분기와 무관합니다.')
